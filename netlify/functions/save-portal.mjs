// /.netlify/functions/save-portal.mjs - Criar novo portal (alias do plural) (requer admin)
...
function checkAdminAuth(event) {
  ...
  const adminPassword = 'admin123'; // igual ao save-portals.mjs
...
export const handler = async (event) => {
  ...
  try {
    ...
    const result = await sql/*sql*/`
      ...
      VALUES (${id}, ${title}, ${desc}, ${url}, ${tagsArray}, ${status || 'ativo'}, ${icon || 'puzzle'}, ${!!pinned}, 1)
      RETURNING id
    `;

    return ok({ ok: true, id: result[0].id, message: 'Portal criado com sucesso' });
  } catch (e) {
    console.error('Erro ao criar portal (alias):', e);
